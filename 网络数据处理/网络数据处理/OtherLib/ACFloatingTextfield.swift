...
@objc open class ACFloatingTextfield: UITextField {
     ...
     @IBInspectable open var lineColor : UIColor = UIColor.black
        {
            didSet{  bottomLineView?.backgroundColor = lineColor }
        }
...
fileprivate extension ACFloatingTextfield {
    ...
    func addBottomLine(){
        ...
        if bottomLineView != nil {
            ...
            //高度约束
            let height:NSLayoutConstraint = NSLayoutConstraint(item: bottomLineView!, attribute: NSLayoutAttribute.height, relatedBy:NSLayoutRelation.equal, toItem:nil, attribute:NSLayoutAttribute.notAnAttribute, multiplier:1.0, constant:2)
            bottomLineView?.addConstraint(height)//自己添加约束
            
            //右边约束
            let right:NSLayoutConstraint = NSLayoutConstraint(item: bottomLineView!, attribute: NSLayoutAttribute.right, relatedBy:NSLayoutRelation.equal, toItem:self, attribute:NSLayoutAttribute.right, multiplier:1.0, constant: 0)
            //右边约束
            let left:NSLayoutConstraint = NSLayoutConstraint(item: bottomLineView!, attribute: NSLayoutAttribute.left, relatedBy:NSLayoutRelation.equal, toItem:self, attribute:NSLayoutAttribute.left, multiplier:1.0, constant: 0)
            
            //下边约束
            let bottom:NSLayoutConstraint = NSLayoutConstraint(item: bottomLineView!, attribute: NSLayoutAttribute.bottom, relatedBy:NSLayoutRelation.equal, toItem:self, attribute:NSLayoutAttribute.bottom, multiplier:1.0, constant: 0)
            
            self.addConstraints([right,left,bottom])
        }
