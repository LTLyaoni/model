...
class WebView: UIView
{
    ...
    fileprivate func setupUI(webConfig:WkwebViewConfig)  {
        ...
        let script = WKUserScript(source: webConfig.js, injectionTime: WKUserScriptInjectionTime.atDocumentEnd, forMainFrameOnly: true)
        ...
        webView.scrollView.mj_header = MJRefreshNormalHeader(refreshingBlock: { [weak self] in
            self?.reload()
        })
...
//    请求链接处理
    ...
    open override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "estimatedProgress"
        {
            ...
            LTLLog(webView.estimatedProgress)
        }
        else if keyPath == "canGoBack"
        {
            self.delegate?.canGoBack(webView)
        }
        else if keyPath == "title"
        {
            LTLLog(webView.title)
            self.delegate?.title(webView)
        }
        else
        {
...
extension WebView: WKNavigationDelegate{
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        if let requestURL = navigationURL?.removingPercentEncoding {
            ...
            else if requestURL.hasPrefix("alipay://")// 支付宝支付
            {
                ...
                if let strEncoding = urlString.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) {
                    ...
                    if let urlalipayURL:URL = URL(string: payString) {
                        if #available(iOS 10.0, *) {
                            UIApplication.shared.open(urlalipayURL, options: [:], completionHandler: { [weak self] result in
                                self?.webView.reload()
                            })
                        } else {
...
//        showHudWithTip(returnError(error._code))
